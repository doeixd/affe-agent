...
| **`AgentInput`** | The typed value a submission is asked with, and the rendering the model sees, kept apart. | The mirror of `AgentOutput`. The value is on the fibre (`Current`, a `Context.Reference` like `CurrentPrincipal`) and on `SubmissionStarted`; the rendering is what enters history. Across a boundary the value crosses encoded as `AgentInput.Typed`, decoded by the host with the session's schema (`AgentClient.typed(agent)` writes the value, never the wire form); `/durable` journals it and renders in the workflow, an Effect-valued renderer as an activity. `Agent.InputOf` extracts the declared type without the invariance trap. |
