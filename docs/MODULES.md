...
| **`AgentLoop`** | The continuation policy: does another turn happen? `Continue`, `Stop`, or `Final` -- one more turn with tools withheld, then stop. | Policy, never engine. It decides; it does not perform. Follow-up state is withheld from it by design. `/budget` is a loop; so are `maxTurns`, `maxToolCalls`, `maxDuration` and `limits`, and any stopping rule. A stop can carry a reason, which reaches `RunCompleted.stopReason`. |
...
| **`Journal`** | Where a run records what it must not do twice: `step(name, schema, effect)`. | The identity locally; an activity under `/durable` (`DurableJournal`). The seam `PLAN.md` §30.1 admitted on 2026-09-26, kept to `step` on purpose. Slice 1 of item 129: the model and tools still go through `/durable`'s substitutions. |
...
| **`AgentInput`** | The typed value a submission is asked with, and the rendering the model sees, kept apart. | The mirror of `AgentOutput`. The value is on the fibre (`Current`, a `Context.Reference` like `CurrentPrincipal`) and on `SubmissionStarted`; the rendering is what enters history. Across a boundary the value crosses as the session's encoded input, one wire shape, decoded by the host with the session's schema (`AgentClient.typed(agent)` writes the value, never the wire form); `/durable` journals it and renders in the workflow, an Effect-valued renderer as an activity. `Agent.InputOf` extracts the declared type without the invariance trap. |
...
| **local provider** | `/sandbox/local` | Node-backed provider. | A host module — its own entry so importing `/sandbox` never pulls `node:*`. Enforced by `scripts/verify-portability.mjs`. |
| **`CloudflareHost`** / **`IsolateExecutor`** | `/cloudflare` | `make({ agent, layer })`: the Durable Object class (one per session; history at every committed turn, events journaled, dispatched work as logical alarms) and the Worker that routes to it. `IsolateExecutor.executor()`: a `CodeExecutor` that runs each program in a Dynamic Worker with no network, calling tools through the object's broker. | The Cloudflare host module, on `effect-cf` (an optional peer) by decision `plan-effect-cf-and-webtransport.md` §3a. Its own compile (`tsconfig.cloudflare.json`), exempted by name in the portability lint, imported only on workerd. |
...
| **`WebToolkit`** | `/web` | Model-facing search, guarded fetch, rendered-page capture and a bounded same-host crawl, over provider-neutral `WebSearch` / `WebFetch` / `WebCapture` / `WebCrawl` capabilities. | Providers are Layers: `/web/brave`, `/web/http`, `/web/cloudflare` (Browser Rendering's REST API, portable). `WebCrawl.layer` is itself portable, over any capture. Test doubles live in `/testing`. |
...
| **`Recovery`** | `/durable` | What a session's reconciliation owes, as a pure `classify` over the stores' evidence; `inspect` explains it to an operator, lists parked tool outcomes, and reports store inconsistencies. | Reads only. `DurableAgentClient` acts on the same decision each time a session is acquired. |
...
| **host-wide events** | (root host) | `AgentSessionHost.hostEvents`: every hosted session's events plus the host's own hosting lifecycle, as one stream. | Per-session order preserved, no host-wide sequence (it would be scheduler order wearing a sequence's clothes). Its own authorized operation. Live-only; `eventLog` stays the finite cursored read, per session. Folded by `SessionProjection` -- see `examples/host-events.ts`. |
| **`SessionProjection`** | `/sessions` | A session's events folded into what is true *now*: lifecycle, submission/run/turn counts, accumulated usage, tool outcomes, open tool calls and unanswered elicitations. | Pure `(state, envelope) => state`, so a live `Stream`, a `DeliveryLog.read({ after })` and an array all fold the same way — which makes repairing a gap a re-fold of the whole log rather than a second code path — not a resume at the gap cursor, which loses everything before it. Folds `DeliveryLog`; does not duplicate it. The `SessionDirectory` this would serve (`effect-plan-2.txt` §26) is not built. |
| **`Messaging`** | `/sessions` | Messages between sessions: named routes, a required `authorize`, replies only to a recorded sender, and a delivery status per message. | A message is a framework `SessionInbox` item, so delivery is the inbox's: durable, deduplicated, never into a submission in flight. Split into a context service the tools read and a `deliverer` built where the client exists, because joining them is a layer cycle. `plan-supervision.md` §2. |
| **`Monitor`** | `/sessions` | One session told when another's submission fails or is interrupted, or when it closes, as a framework item in its inbox. | A down's id is its event's coordinates, so a duplicate observation enqueues once. It uses `Messaging`'s queue by default, so one delivery loop carries both. `plan-supervision.md` §3. |
| **`Supervisor`** | `/sessions` | OTP-style supervision in process: children with restart types, the three strategies, intensity, a shared token ceiling, and escalation up a tree. `task` makes an agent a child, `fresh` or `resubmit`. Given `onGiveUp`, it consults an agent through `control`'s tools where it would give up, or on every exit its classifier answers `"ask"`; the agent can restart, stop, steer, and start children from templates. | Children are effects, so tasks, processes and nested supervisors share one policy. The failure decides whether to restart: by default only a retryable `AiError`, and never an unknown tool outcome. `plan-supervision.md` §4. |
