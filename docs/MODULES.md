...
| **`CloudflareHost`** | `/cloudflare` | `make({ agent, layer })`: the Durable Object class (one per session; history at every committed turn, events journaled, dispatched work as logical alarms) and the Worker that routes to it. | The Cloudflare host module, on `effect-cf` (an optional peer) by decision `plan-effect-cf-and-webtransport.md` §3a. Its own compile (`tsconfig.cloudflare.json`), exempted by name in the portability lint, imported only on workerd. |
