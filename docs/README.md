...
| [open-decisions.md](./open-decisions.md) | What is still undecided, and how each gets resolved: the owner's calls (push, merged branches, the stash, a key for live runs, what item 113 becomes), the ones decided under delegation, and the parked items with the trigger that reopens each. Entries are deleted once decided. |
