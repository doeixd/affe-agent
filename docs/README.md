...
| [plan-subagent-execution-forms.md](./plan-subagent-execution-forms.md) | The three subagent execution forms from the `danieljvdm/effect-agent` surface -- attached, durable attached, background -- as one design: a subagent is a tool that opens a child session, and the forms differ by host and lifetime, not by execution model. Maps each effect-agent concept to a seam, records the deliberate divergences (no detached work; a ping-back is never an implicit follow-up; typed failures, not data flags), sequences the work, and gates each slice on a caller. |
| [plan-auto-model-routing.md](./plan-auto-model-routing.md) | `AutoModel` from the `danieljvdm/effect-agent` subagent surface: a routing `LanguageModel` layer that chooses a model from the request, with a per-session selection store so a follow-up keeps its child's choice and siblings choose independently. Proposal, gated on a second caller; recommends a pure `choose` so a replay picks the same model with no new journal entry. |
| [plan-background-delegation.md](./plan-background-delegation.md) | `Subagent.background`: a child that outlives the parent, with `start` / `follow-up` tools and reports delivered back through `/sessions`' `SessionInbox`. Proposal, gated on a caller; the child is a named, enumerable session -- never a detached fiber -- and its durable half depends on item 113's child-workflow delegation. |
