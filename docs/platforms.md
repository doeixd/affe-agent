...
| **History** | in memory; `/durable` rebuilds it from the journal, `/tree` persists it to a `NodeStore` | DO SQLite, written as each submission completes and restored when the object wakes |
| **Work that survives the process** | `/durable`: model and tool calls are Workflow activities, so a resumed submission replays them instead of repeating them; `/cluster` reassigns a shard when its owner dies mid-activity | the *conversation* survives hibernation and death; a run in flight does not — the DO equivalent of Node without `/durable` (see the file's header for why the workflow engine is not used there) |
...
| **Scheduling** | `/scheduling` over Effect's `Schedule`; `/cluster`'s `ScheduledAgent` across nodes | none yet; a DO-alarm `AgentDispatcher` is item 39 of [remaining-work.md](./remaining-work.md) |
