...
| **13** ✅ | Add `/coding` cumulative file-operation details (`CodingSummary.wrap` composes around any `Summarise`: a machine-owned `## Files touched` section computed from the folded stretch's own read_file/write_file/edit_file calls, unioned with the previous summary's section and with sections inside folded carryover messages -- accumulation through the summary text itself, so no widened `SummaryResult` and no checkpoint-schema change; §22's first-version compromise of knowing our own toolkit's names, stated) |
...
| **15** ✅ | Only after all that, investigate automatic provider-overflow recovery (`plan-context-lessons.md` 2.1, ledger 60d: a `Rollover` checkpoint beside `Summary` -- protected prefix, window marker, handoff, retained tail, no summariser -- on the model's `new_context` request or as `onCannotHelp: "rollover"` when a summary will not fit; the provider-overflow *trigger* itself waits on a classification Effect's AI layer does not offer, item 60d-i) |
