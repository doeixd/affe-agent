...
session (`DurableSubmission`), over `DurableToolkit.delegate`, its session id
(`Subagent.childSessionId`) a pure function of the conversation and the call so
a host can find it); `/state`; `/skills`; `/memory`; `/evals`;
