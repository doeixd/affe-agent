...
| `create-session-request.json` | this commit | `test/CreateSessionWire.test.ts` | `AgentProtocol.CreateSessionRequest` unseeded -- the shape every client sent before `history` existed -- and seeded with a system and a user message in the prompt wire encoding. Asserted byte-identical both ways, and that the unseeded one decodes with no history. |
...
| `admission-descriptions.json` | `cbb33ff`, measuring `725551e` and `5e13679` | `test/BehaviorFixtures.test.ts` | What a durable run journals of the permission policy and host scheduling it was admitted under: a policy with a case-insensitive pattern (described `regexp/i:`, a flagless one as before), a re-creatable scheduling, and a `Serialize`. Asserted equal to what the code describes now, and that the recorded policy re-creates to the same decisions and the `Serialize` does not re-create. |
| `channel-input-table.json` | item 120 | `test/DurableChannelsCrash.test.ts` | The columns of the channel-input table `DurableChannels.sqlStoreWithTable` creates, `claimed_by` included: a drain claims the rows it takes, so a re-execution after a crash takes them again. A deployment that manages its own schema must provide exactly this; a change here is a storage change. |
